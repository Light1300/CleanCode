...
// ---------------------------
// Middleware: Verify JWT (only for authenticated routes)
const authMiddleware = async (c: any, next: any) => {
  ...
  try {
    ...
    if (!payload?.id) return c.json({ error: "Invalid token" }, 401);
...
blogRouter.post("/", authMiddleware, async (c) => {
  ...
  try {
    ...
    return c.json({ error: "Failed to create post", details: `${err}` }, 500);
  }
});
...
blogRouter.put("/", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (updated.count === 0) return c.json({ error: "Blog not found or unauthorized" }, 404);
